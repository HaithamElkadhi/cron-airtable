...
Single script **`main.py`**: reads `.env`, calls the Airtable REST API, prints **Prospect Situation** counts (**Lost**, **Lead**, **Prospect**, **Candidate**, **Student**), then either asks whether to write **KPIS** (`UPDATE_KPIS=prompt`, default), always writes (`true`), or never (`false`). Without **`KPI_RECORD_ID`**, each write **creates a new** KPIS row; with **`KPI_RECORD_ID`**, that row is **updated**. KPIS fields: **Lost**, **Lead**, **Prospect**, **Candidate**, **Student**, **Total_Prospect**.
...
- **`AIRTABLE_API_KEY`** — access must include both the Prospects and KPIS tables.
